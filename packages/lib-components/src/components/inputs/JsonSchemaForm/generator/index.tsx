...
export const SchemaGenerator = forwardRef<
  HTMLFormElement,
  ISchemaGeneratorProps
>(({ initialSchema, onSubmit }, ref) => {
  const { control, handleSubmit, getValues, resetField } = useForm<IFormInputs>(
    {
      ...
      defaultValues: {
        configFields: initialSchema
          ? schemaToInputs(initialSchema)
          : [{} as Input],
      },
    }
  );
  ...
  useEffect(() => {
    if (!configFields?.length && !initialSchema) {
      append({
        name: 'Config field 1',
