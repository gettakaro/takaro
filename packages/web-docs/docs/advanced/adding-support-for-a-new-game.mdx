...
- `platformId` (string): Generic platform identifier
- `ip` (string): Player's IP address
- `ping` (number): Player's connection ping

Game connectors should populate the appropriate platform-specific fields based on how players authenticate in their game. For example, a Steam-based game would populate `steamId`, while an Epic Games Store game would populate `epicOnlineServicesId`.
...
```json
{
  ...
  "payload": [
    ...
    },
    {
      ...
      "name": "PlayerName2",
      "steamId": null,
      "epicOnlineServicesId": "abc123def456",
      "xboxLiveId": null,
      "platformId": "epic_67890",
      "ip": "192.168.1.2",
      "ping": 75
    }
  ],
